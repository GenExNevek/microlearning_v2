...
class MockStrategyA:
    ...
    def extract(self, pdf_document, img_info, page_num, extraction_info):
        if 'details' not in extraction_info or not isinstance(extraction_info.get('details'), dict):
            extraction_info['details'] = {}
        extraction_info['strategy_name_in_mock_class'] = 'A' # To distinguish if ever called
...
class MockStrategyB:
    ...
    def extract(self, pdf_document, img_info, page_num, extraction_info):
        if 'details' not in extraction_info or not isinstance(extraction_info.get('details'), dict):
            extraction_info['details'] = {}
        extraction_info['strategy_name_in_mock_class'] = 'B'
        extraction_info['success'] = True
...
class MockStrategyC:
    ...
    def extract(self, pdf_document, img_info, page_num, extraction_info):
        if 'details' not in extraction_info or not isinstance(extraction_info.get('details'), dict):
            extraction_info['details'] = {}
        extraction_info['strategy_name_in_mock_class'] = 'C'
        extraction_info['success'] = True
...
class TestRetryCoordinator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # self.strategies is removed from here. It will be constructed within each test
        # using the patched strategy classes.
    ...
    def test_first_strategy_succeeds(self, PatchedMockStrategyA, PatchedMockStrategyB, PatchedMockStrategyC):
        ...
        mock_instance_a.extract.return_value = (MagicMock(), {'success': True, 'details': {'dimensions': '100x100', 'mode': 'RGB'}})
        PatchedMockStrategyA.return_value = mock_instance_a

        # Configure StrategyB and C (their class mocks will return instances, but extract won't be called)
        ...
        PatchedMockStrategyB.assert_not_called()
        mock_instance_b.extract.assert_not_called()
        PatchedMockStrategyC.assert_not_called()
        mock_instance_c.extract.assert_not_called()
    ...
    def test_second_strategy_succeeds_after_first_fails(self, PatchedMockStrategyA, PatchedMockStrategyB, PatchedMockStrategyC):
        ...
        mock_instance_a = MagicMock(spec=MockStrategyA)
        mock_instance_a.extract.return_value = (None, {'success': False, 'error': 'A failed'})
        PatchedMockStrategyA.return_value = mock_instance_a

        mock_instance_b = MagicMock(spec=MockStrategyB)
        mock_instance_b.extract.return_value = (MagicMock(), {'success': True, 'details': {'dimensions': '200x200', 'mode': 'L'}})
        PatchedMockStrategyB.return_value = mock_instance_b

        mock_instance_c = MagicMock(spec=MockStrategyC)
        PatchedMockStrategyC.return_value = mock_instance_c # Will not be called
    ...
    def test_all_strategies_fail(self, PatchedMockStrategyA, PatchedMockStrategyB, PatchedMockStrategyC):
        ...
        mock_instance_b = MagicMock(spec=MockStrategyB)
        mock_instance_b.extract.return_value = (None, {'success': False, 'error': 'B failed', 'issue_type': 'size_issues'})
        PatchedMockStrategyB.return_value = mock_instance_b

        mock_instance_c = MagicMock(spec=MockStrategyC)
        mock_instance_c.extract.return_value = (None, {'success': False, 'error': 'C failed', 'issue_type': 'extraction_failed'})
        PatchedMockStrategyC.return_value = mock_instance_c
        ...
        self.assertEqual(extraction_info['issue_type'], 'extraction_failed') # This should be the issue_type of the last failed attempt if not overridden
    ...
    def test_retries_disabled(self, PatchedMockStrategyA, PatchedMockStrategyB, PatchedMockStrategyC):
        ...
        strategies_for_coordinator = [
            (PatchedMockStrategyA, 'strategy_a'),
            (PatchedMockStrategyB, 'strategy_b'), # Will be available but not used
            (PatchedMockStrategyC, 'strategy_c'), # Will be available but not used
        ]
    ...
    def test_initial_extraction_info_is_merged(self, PatchedMockStrategyA, PatchedMockStrategyB):
        ...
        mock_instance_b = MagicMock(spec=MockStrategyB) 
        PatchedMockStrategyB.return_value = mock_instance_b # Won't be called
